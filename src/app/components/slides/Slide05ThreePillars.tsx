...
export function Slide05ThreePillars() {
  ...
  return (
    <div className="w-full h-full min-h-screen flex flex-col items-center justify-center bg-[#0a0a0a] text-white p-4 relative overflow-hidden">
      ...
      <motion.div
        ...
        className="text-center mb-8 relative z-10 pt-10" // Уменьшил отступ снизу
      >
      ...
      {/* Main Content Area */}
      <div className="relative w-full max-w-6xl flex-1 min-h-[600px] flex items-center justify-center">
        ...
        >
          ...
          <div className="w-32 h-32 md:w-40 md:h-40 rounded-full bg-gradient-to-br from-[#1a1a1a] to-black border border-blue-500/30 flex flex-col items-center justify-center shadow-[0_0_40px_rgba(59,130,246,0.2)]">
            <span className="text-white text-sm font-light tracking-wider opacity-60">Ozimiz</span>
            <span className="text-blue-400 text-xl md:text-2xl font-medium tracking-tight">.Control</span>
          </div>
        </motion.div>
        ...
        {pillars.map((pillar, pillarIndex) => {
          ...
          const radius = 230; // Уменьшил радиус, чтобы влезало
          ...
          return (
            <div key={pillar.title} className="absolute top-1/2 left-1/2 w-0 h-0">
              ...
              >
                ...
                <motion.div
                  className="absolute"
                  style={{ left: radius, transform: `rotate(${-angleDeg}deg)` }}
                >
                  <motion.div
                  ...
                  >
                    <div className={`w-52 md:w-60 rounded-3xl bg-gradient-to-br ${pillar.color} p-[1px] ${pillar.shadow} shadow-lg`}>
                      <div className="w-full h-full bg-black/95 backdrop-blur-md rounded-3xl p-4 md:p-5 relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          <div className={`p-2 rounded-xl bg-gradient-to-br ${pillar.color} bg-opacity-10`}>
                            <Icon className="w-5 h-5 md:w-6 md:h-6 text-white" />
                          </div>
                          <h3 className="text-lg md:text-xl text-white font-light tracking-wide">{pillar.title}</h3>
                        </div>
                          ...
                          {pillar.items.map((item, itemIndex) => {
                            ...
                            return (
                              <motion.div
                                ...
                                className="flex items-center gap-2 md:gap-3 text-white/60 text-xs md:text-sm group cursor-default hover:text-white transition-colors"
                              >
                                <SubIcon className="w-3 h-3 md:w-4 md:h-4 text-white/40 group-hover:text-blue-400 transition-colors" />
                                <span>{item}</span>
                              </motion.div>
                            );
                          })}
